...
This guide walks you through the steps for 
  ...
  2) Make REST Call
  3) Dockerfile, changes in project files trigger dockerhub build (container1616/gs-scheduling-tasks)

== What you'll build

You'll build an application that prints out REST call response every 5 seconds using Spring's `@Scheduled` annotation.
